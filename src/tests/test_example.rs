...
fn query_monsters()
{
	...
	let bindings = ~[~"name", ~"weight", ~"announcement", ~"subject", ~"min", ~"max", ~"habitat"];
	let expected = Solution {namespaces: ~[], bindings: bindings, num_selected: 3, rows: ~[
		~[@StringValue(~"Lich", ~""), @IntValue(3), @StringValue(~"You feel a chill.", ~"")],
		~[@StringValue(~"Mummy", ~""), @IntValue(2), @UnboundValue],
		~[@StringValue(~"Necromancer", ~""), @IntValue(2), @UnboundValue],
	]};
